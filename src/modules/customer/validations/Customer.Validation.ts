...
export const validateLeadData = (data: any) => {
    ...
    if (data.leadEmail && !emailRegex.test(data.leadEmail)) throw new Error("Email không hợp lệ");
    if (data.leadTaxId && !taxIdRegex.test(data.leadTaxId)) throw new Error("Mã số thuế  không hợp lệ (Phải là 10 số hoặc 13 số định dạng XXXXXXXXXX-XXX)");
    if (data.leadPhone && !phoneRegex.test(data.leadPhone)) throw new Error("Số điện thoại không hợp lệ");
};
